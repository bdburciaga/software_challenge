...
import com.burciaga.plutotv.softwarechallenge.utils.validation.WebVttValidator;
...
class WebVttControllerTest {
    ...
    WebVttController webVttController = new WebVttController(validator, parser);
    ...
    private final String WEBVTT_CONTENTS = "WEBVTT\n" +
            ...
            "\n" +
            "00:03:04.358 --> 00:03:04 line:-2\n" +
            "with other\n" +
            "moderately wealthy people";

    @Test
    void validateWebVtt() {
        ResponseEntity<ValidationResult> result = webVttController.validateWebVtt(WEBVTT_CONTENTS);
        assertFalse(result.getBody().isValid());
        assertTrue(result.getBody().getSuggestions().size() > 0);
    }

    @Test
    void editWebVtt() {
        CuePoint cuePoint = new CuePoint();
        cuePoint.setLineNumber(198);
        cuePoint.setStartTime("01:23:45.678");
        cuePoint.setEndTime("22:33:44");

        EditRequest editRequest = new EditRequest();
        editRequest.setWebVtt(WEBVTT_CONTENTS);
        editRequest.setCuePoints(Arrays.asList(cuePoint));

        ResponseEntity<EditResult> editResult = webVttController.editWebVtt(editRequest);
        assertNotEquals(editResult.getBody().getUpdatedContent(), WEBVTT_CONTENTS);
        assertTrue(editResult.getBody().getMessage().isBlank());
    }

    @Test
    void editWebVttInvalidLine() {
        int lineNumber = 199;
        CuePoint cuePoint = new CuePoint();
        cuePoint.setLineNumber(lineNumber);
        cuePoint.setStartTime("01:23:45.678");
        cuePoint.setEndTime("22:33:44");
        ...
        ResponseEntity<EditResult> editResult = webVttController.editWebVtt(editRequest);
        assertEquals(editResult.getBody().getUpdatedContent(), WEBVTT_CONTENTS);
        assertTrue(editResult.getBody().getMessage().contains(String.valueOf(lineNumber)));
    }

    @Test
    void editWebVttOutOfBoundsLineNumber() {
        int lineNumber = 502;
        CuePoint cuePoint = new CuePoint();
        cuePoint.setLineNumber(lineNumber);
        cuePoint.setStartTime("01:23:45.678");
